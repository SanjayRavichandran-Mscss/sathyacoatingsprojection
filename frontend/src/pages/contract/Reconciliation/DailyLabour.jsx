    ...
    if (selectedDescription) {
      const fetchData = async () => {
        try {
          setLoadingData(true);
          setError(null);
          const response = await axios.get("http://103.118.158.127/api/site-incharge/calculate-labour-budget");
          setLabourBudget(response.data.processed_po_budgets || []);
        } catch (err) {
          console.error("Failed to load labour data:", err);
          setError("Failed to load labour data. Please ensure the backend server is running on http://localhost:5000.");
        } finally {
