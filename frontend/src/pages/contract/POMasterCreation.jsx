...
const POMasterCreation = ({
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        const inchargeResponse = await axios.get("http://103.118.158.33/api/reckoner/incharge-types");
        setInchargeTypes(
        ...
        const locationResponse = await axios.get("http://103.118.158.33/api/reckoner/locations");
        setLocations(
        ...
        const reckonerTypeResponse = await axios.get("http://103.118.158.33/api/reckoner/reckoner-types");
        setReckonerTypes(
      ...
      const fetchSites = async () => {
        try {
          ...
          const response = await axios.get(
            `http://103.118.158.33/api/reckoner/sites-by-company/${selectedCompanyId}`
          );
          ...
          const siteReckonerPromises = sitesData.map(async (site) => {
            try {
              const reckonerResponse = await axios.get(
                `http://103.118.158.33/api/reckoner/site-reckoner/${site.site_id}`
              );
  ...
  const handleUpdateSite = async (siteId) => {
    try {
      ...
      await axios.put(`http://103.118.158.33/api/reckoner/sites/${siteId}`, updateData);
  ...
  const handleCreateCategory = async (categoryName) => {
    try {
      const response = await axios.post("http://103.118.158.33/api/reckoner/categories", {
  ...
  const handleCreateWorkItem = async (descName) => {
    try {
      const response = await axios.post("http://103.118.158.33/api/reckoner/work-items", {
        desc_name: descName,
      });
  ...
  const handleCreateSubcategory = async (categoryIndex, itemIndex) => {
    ...
    try {
      const response = await axios.post("http://103.118.158.33/api/reckoner/subcategories", {
