    ...
    const fetchEmployees = async () => {
      ...
      try {
        const [empResponse, labourResponse] = await Promise.all([
          axios.get("http://103.118.158.33/api/site-incharge/employees"),
          axios.get("http://103.118.158.33/api/admin/labour"),
        ]);
        if (isMounted) {
  ...
  useEffect(() => {
    if (selectedProject && selectedSite) {
      ...
      const fetchWorkDescriptions = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`http://103.118.158.33/api/site-incharge/work-descriptions?site_id=${selectedSite.value}`);
  ...
  const handleSaveAssignments = async () => {
    ...
    try {
        if (hasIncharge) {
            ...
            await axios.post("http://103.118.158.33/api/material/assign-incharge", inchargePayload);
        ...
        if (hasLabour) {
            ...
            await axios.post("http://103.118.158.33/api/site-incharge/save-labour-assignment", labourPayload);
