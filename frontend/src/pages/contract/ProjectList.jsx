...
const ProjectList = () => {
  ...
  const fetchWorkDescriptions = async (siteId) => {
    try {
      const res = await axios.get(`http://103.118.158.33/api/project/work-descriptions-by-site/${siteId}`);
  ...
  const fetchSiteInfo = async (poNumber) => {
    try {
      setLoadingSite(true);
      const res = await axios.get(`http://103.118.158.33/api/reckoner/sites/${poNumber}`);
      if (res.data.success) {
  ...
  const handleSubmit = async (rec_id) => {
    try {
      ...
      await axios.put(`http://103.118.158.33/api/reckoner/completion_status/${rec_id}`, payload);
