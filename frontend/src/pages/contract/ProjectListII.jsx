...
const { encodedUserId } = useParams();
  ...
  const fetchWorkDescriptions = async (siteId) => {
    try {
      const res = await axios.get(`https://scpl.kggeniuslabs.com/api/project/work-descriptions-by-site/${siteId}`);
  ...
  const fetchSiteInfo = async (poNumber) => {
    try {
      setLoadingSite(true);
      const res = await axios.get(`https://scpl.kggeniuslabs.com/api/reckoner/sites/${poNumber}`);
      if (res.data.success) {
  ...
  const handleSubmit = async (rec_id) => {
    try {
      ...
      await axios.put(`https://scpl.kggeniuslabs.com/api/reckoner/completion_status/${rec_id}`, payload);
