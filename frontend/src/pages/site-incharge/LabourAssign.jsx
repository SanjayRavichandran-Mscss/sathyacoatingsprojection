...
const LabourAssign = () => {
  ...
  const handleViewAssignedLabours = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `http://103.118.158.127/api/site-incharge/assigned-labours?project_id=${selectedProject.value}&site_id=${selectedSite.value}&desc_id=${selectedWorkDesc.value}`
      );
  ...
  const handleUpdateAssignment = async (assignmentId) => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://103.118.158.127/api/site-incharge/update-labour-assignment",
        payload
      );
      toast.success(response.data.message);
