...
const SupplyProjectCreation = ({ companyId, onClose, onProjectCreated }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [companyResponse, projectsResponse, locationsResponse, reckonerTypesResponse] = await Promise.all([
        axios.get(`http://103.118.158.33/api/supply/companies/${companyId}`),
        axios.get(`http://103.118.158.33/api/supply/projects/${companyId}`),
        axios.get(`http://103.118.158.33/api/supply/locations`),
        axios.get(`http://103.118.158.33/api/supply/reckoner-types`),
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let pd_id;
      if (isNewProject) {
        const projectResponse = await axios.post("http://103.118.158.33/api/supply/create-project", {
      ...
      await axios.post("http://103.118.158.33/api/supply/create-site", siteData);
