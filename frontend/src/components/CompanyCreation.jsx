...
const CompanyCreation = ({ onCompanyCreated, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const statesRes = await axios.get("https://scpl.kggeniuslabs.com/api/project/states");
        setStates(statesRes.data.data.map(s => ({ id: s.id, name: s.state_name })));
        
        const citiesRes = await axios.get("https://scpl.kggeniuslabs.com/api/project/cities");
  ...
  const addNewState = async (name) => {
    try {
      const res = await axios.post("https://scpl.kggeniuslabs.com/api/project/create-state", { state_name: name });
  ...
  const addNewCity = async (name) => {
    try {
      const res = await axios.post("https://scpl.kggeniuslabs.com/api/project/create-city", { city_name: name });
      const newId = res.data.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("https://scpl.kggeniuslabs.com/api/project/create-company", formData);
      setFormData({
        company_name: "",
        address: "",
        gst_number: "",
