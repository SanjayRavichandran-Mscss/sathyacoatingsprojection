...
const CompanyCreationSupply = ({ onCompanyCreated, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const statesRes = await axios.get("https://scpl.kggeniuslabs.com/api/supply/states");
        setStates(statesRes.data.data.map(s => ({ id: s.id, name: s.state_name })));
        
        const citiesRes = await axios.get("https://scpl.kggeniuslabs.com/api/supply/cities");
  ...
  const addNewState = async (name) => {
    try {
      const res = await axios.post("https://scpl.kggeniuslabs.com/api/supply/create-state", { state_name: name });
  ...
  const addNewCity = async (name) => {
    try {
      const res = await axios.post("https://scpl.kggeniuslabs.com/api/supply/create-city", { city_name: name });
      const newId = res.data.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("https://scpl.kggeniuslabs.com/api/supply/create-company", payload);
      setFormData({
        company_name: "",
        address: "",
        gst_number: "",
      ...
      const errorMsg = error.response?.data?.error || "Failed to create supply company. Please try again.";
      setError(errorMsg);
      Swal.fire({
        icon: "error",
        title: "Error",
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl p-6 transform transition-all">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h3 className="text-2xl font-bold text-gray-900">Create Supply Client</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Supply Client Name*</label>
                <input
                  type="text"
                  name="company_name"
          ...
          <div className="flex justify-end space-x-4 mt-8">
            ...
            >
              ...
              <span>{loading ? "Creating..." : "Create Supply Client Master"}</span>
            </button>
          </div>
        </form>
      </div>
...
export default CompanyCreationSupply;