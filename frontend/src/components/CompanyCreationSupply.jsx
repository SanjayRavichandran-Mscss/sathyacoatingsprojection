...
const CompanyCreationSupply = ({ onCompanyCreated, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const statesRes = await axios.get("http://103.118.158.33/api/supply/states");
        setStates(statesRes.data.data.map(s => ({ id: s.id, name: s.state_name })));
        
        const citiesRes = await axios.get("http://103.118.158.33/api/supply/cities");
  ...
  const addNewState = async (name) => {
    try {
      const res = await axios.post("http://103.118.158.33/api/supply/create-state", { state_name: name });
  ...
  const addNewCity = async (name) => {
    try {
      const res = await axios.post("http://103.118.158.33/api/supply/create-city", { city_name: name });
      const newId = res.data.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://103.118.158.33/api/supply/create-company", payload);
      setFormData({
        company_name: "",
        address: "",
        gst_number: "",
